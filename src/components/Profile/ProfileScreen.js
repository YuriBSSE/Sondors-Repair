...
const ProfileScreen = ({ route }) => {
    ...
    return (
        <View style={tailwind('flex flex-col px-6')}>
         ...
         <Text left sm tertiary style={tailwind('mt-1')}>Name</Text>
            <View style={tailwind('mt-2')}>
            <Text left lg  style={tailwind('mt-1')}>{profile?.name}</Text>
        </View>
        </View>
        <View style={tailwind('mt-4')}>
         <Text left sm tertiary style={tailwind('mt-1')}>Email</Text>
            <View style={tailwind('mt-2')}>
            <Text left lg  style={tailwind('mt-1')}>{profile?.email}</Text>
        </View>
        </View>
        <View style={tailwind('mt-4')}>
         <Text left sm tertiary style={tailwind('mt-1')}>Total Jobs</Text>
            <View style={tailwind('mt-2')}>
            <Text left lg  style={tailwind('mt-1')}>{profile?.totalJobs}</Text>
        </View>
        </View>
        <View style={tailwind('mt-4')}>
            <Rating disabled={true} defaultRating={totalRatting || 0} />
         
            <Text left sm tertiary style={tailwind('mt-1')}>Member since {moment(profile.createdAt).format("LL")}</Text>
        </View>
