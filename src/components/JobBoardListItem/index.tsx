    ...
    return (
        <TouchableOpacity onPress={onPress} style={tailwind('py-3 px-5')}>
            <View style={tailwind('flex-row justify-between')}>
                <Text md left normal>{job.title}</Text>
                <View style={tailwind('flex-row')}>
                    <Text style={{ marginRight: 12 }} sm right tertiary>
                        {moment(job.createdAt).format('LL')}
                    </Text>
                    <FontAwesome name="angle-right" size={16} color={Colors.tertiaryText} />
                </View>
            </View>
            <View style={tailwind('flex-col mt-2')}>
                <Text md left tertiary>{truncate(job.description, 100)}</Text>
                <View style={{display: 'flex', flexWrap: 'wrap'}}>
                <TouchableOpacity style={styles.tagStyle}>
                    <Text left hyperlink bold onPress={onPress}>{job.type}</Text>
                </TouchableOpacity>
                </View>
            </View>
        </TouchableOpacity>
