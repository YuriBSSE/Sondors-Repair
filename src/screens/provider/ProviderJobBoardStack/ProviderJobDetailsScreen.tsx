...
const ProviderJobDetailsScreen = ({ route, navigation }: Props) => {
    ...
    const { headerRightTitle, headerLeftTitle, headerRightOnPress, jobDetails, data, uidC } = route.params;
    ...
    useEffect(() => {
        ...
        navigation.setOptions({
            ...
            headerRight: () => <HeaderRight onPress={headerRightOnPress!} title={headerRightTitle!} />,
        });
    }, []);
    ...
    const createPrivateChanel = async () => {
        const { currentUser } = auth
        const channel = streamClient.channel('messaging', `${currentUser?.uid + jobDetails.streamChatId!}`, {
    ...
    const onApply = async () => {
        ...
        await updateDoc(applyOnJobRef,
            {
                data: [...data, { ...coverLetter, externalId: currentUser?.uid + jobDetails.streamChatId!, uidP: currentUser?.uid, createAt: `${new Date().toString()}`, jobResponseType: 'applied' }]
            }).then(() => {
                createPrivateChanel()
            }).then(() => {
                setModal(false)
    ...
    return (
        <SafeAreaView style={tailwind('flex bg-white items-center justify-center h-full')}>
            ...
            <AppModal modalVisible={isModal}>
                <Text lg bold title style={{ alignSelf: 'flex-start' }}>Apply For Job</Text>
                <View style={[tailwind('w-full mt-2 mb-2')]}>
                    <Text lg style={{ alignSelf: 'flex-start' }}>Cover Title</Text>
                    <TextInput onChangeText={(text) => handelApplyJob('title', text)} style={{ width: '100%', marginTop: 4 }} />
                </View>
                <View style={[tailwind('w-full mt-2')]}>
                    <Text lg style={{ alignSelf: 'flex-start' }}>Cover Letter</Text>
                    <TextInput onChangeText={(text) => handelApplyJob('subtitle', text)} multiline style={{ width: '100%', marginTop: 4 }} />
                </View>
                <View style={[tailwind('w-full mt-2'), { display: 'flex' }]}>
                    <Button loading={loading} onPress={onApply} style={[tailwind('rounded'), { marginTop: 16 }]} title={'Apply'} />
                    <Button onPress={() => {setModal(false)
                   
                    }} secondary style={[tailwind('rounded'), { marginTop: 12 }]} title={'Cancel'} />
                </View>
            </AppModal>
        </SafeAreaView>
    );
